...
- pending_deliveries：尚未消费的投递记录（直接引用，非 id）——消费
  路径 O(k)、无需反向查表；事件身份追踪（消费记录 = fire 的 consumed）
...
class DataPortState:
    ...
    # 粘性锁存（sticky latch）：一旦为 True 永不复位。不能从
    # pending_deliveries 推导——消费即清空，而动态资格必须跨 epoch 保持。
