"""内置验证原语节点:内核语义的"关键字/基础指令",不为业务功能。

定位(2026-08-21,自举边界):本包是节点协议 ABI 的 builtin 实现者——与外部
节点包地位完全相同:直接构造 NodeType 值,经宿主 types 字典注册
...
所有原语统一声明节点级 enable SignalIn(结构级门控验证)——本包自身惯例,
非 ABI 强制(graph-node-protocol.md §2)。
"""
