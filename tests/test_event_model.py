"""事件模型:身份、生命周期、谁生产谁消费、档案保留(DSL v2 迁移后)。
...
def test_event_identity_and_producer():
    ...
    world = make_world(g)
    world.run([
        Injection("sink", "consume.value", SLOT_DATA, Kind.DATA, "x"),
    ...
    injected, trigger, produced = evs
    assert injected.producer is None  # 宿主注入
    assert injected.port == "consume.value"
    assert trigger.producer is None and trigger.port == "tick.trigger"  # 触发注入也是宿主事件
    assert produced.producer == "src"  # 节点产出
    assert produced.port == "tick"
...
def test_event_lifecycle_produced_delivered_consumed():
    ...
    produced = world.timeline.events[2]  # 1=触发注入, 2=src 产出
    assert produced.status == "consumed"  # 全部投递已被消费
    assert len(produced.deliveries) == 1
    d = produced.deliveries[0]
    assert (d.node, d.port, d.slot) == ("sink", "consume.value", "data")
    ...
    assert produced.consumed_by == [(fire_seq, "sink", "consume.value")]
...
def test_fanout_event_multiple_deliveries_and_consumers():
    ...
    ev_out1 = next(e for e in world.timeline.events.values() if e.port == "fan.out1")
...
def test_orphan_event_when_output_unwired():
    ...
    g.add_node("src", "Source")  # tick 未连线
    world = make_world(g)
    world.run([Injection("src", "tick.trigger", SLOT_TRIGGER, Kind.SIGNAL, True)])
    ev = world.timeline.events[2]  # 1=触发注入(被消费), 2=src 产出
...
def test_multiple_unconsumed_events_consumed_together():
    ...
    world.run(
        [
            Injection("sink", "consume.value", SLOT_DATA, Kind.DATA, 1),
            Injection("sink", "consume.value", SLOT_DATA, Kind.DATA, 2),
            Injection("sink", "consume.value", SLOT_DATA, Kind.DATA, 3),
        ]
    )
...
def test_signal_event_dual_role_consumption():
    """信号的两重语义分别消费:同一信号事件扇出到信号端口 + 触发端口,
    一次执行消费两处投递(控制电平与激活各自成对)。"""
    ...
    world = make_world(g)
    world.run([Injection("gate", "convert.data", SLOT_DATA, Kind.DATA, 1)])
    signal_ev = next(e for e in world.timeline.events.values() if e.producer == "gate")
    assert len(signal_ev.deliveries) == 2  # 一次产出,两处投递
    assert {d.port for d in signal_ev.deliveries} == {"pass_value.gate", "pass_value.pass"}
    assert signal_ev.status == "consumed"
    # 门控信号作为控制状态消费;触发端口作为激活消费——各有一条消费记录
    assert {(c[1], c[2]) for c in signal_ev.consumed_by} == {("stod", "pass_value.gate"), ("stod", "pass_value.pass")}
...
def test_archive_retains_consumed_events_across_epochs():
    ...
    world.run([Injection("sink", "consume.value", SLOT_DATA, Kind.DATA, 2)])
    world.run([Injection("sink", "consume.value", SLOT_DATA, Kind.DATA, 3)])
...
def test_fanout_shares_payload_reference():
    ...
    world = make_world(g)
    payload = ["mutable"]
    world.run([Injection("split", "fan.value", SLOT_DATA, Kind.DATA, payload)])
    out1 = next(e for e in world.timeline.events.values() if e.port == "fan.out1")
    out2 = next(e for e in world.timeline.events.values() if e.port == "fan.out2")
